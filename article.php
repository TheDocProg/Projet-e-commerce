...
if (isset($_GET) && !empty($_GET))
{
  ...
  /********** Récuperation des commentaires et des users **********/

  $connexion = getConnexion();

  $sql = $connexion->prepare('SELECT comments.id, comments.title, comments.content, comments.user_id, comments.created_at, users.first_name
                              FROM comments
...
if (isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['quantity']) && !empty($_POST['quantity']))
{
  ...
  if (isset($_SESSION['cart']) && !empty($_SESSION['cart']))
  {
    ...
    foreach($_SESSION['cart'] as $verify)
    {
      ...
      if(strpos($new[0], $_POST['id']) !== false)
      {
